...
class TestPyin(TestCase):

    def testEmpty(self):
        self.assertComputeFails(Pyin(), [])

    def testZero(self):
        pitch, confidence = Pyin()(zeros(1024))
    ...
    def runTest(self, signal, sr, freq, pitch_precision = 1, conf_precision = 0.1):
        ...
        pitchDetect = Pyin(frameSize=frameSize, sampleRate = sr)
        pitch = []
        confidence = []
        for frame in frames:
    ...
    def testInvalidParam(self):
        self.assertConfigureFails(Pyin(), {'frameSize' : 1})
        self.assertConfigureFails(Pyin(), {'sampleRate' : 0})
#
